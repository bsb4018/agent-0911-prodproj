...
class LLMRegistry:
    ...
    LLMS: List[Dict[str, Any]] = [
        ...
        },
        {
            ...
            "llm": ChatOpenAI(
                model="gpt-4o",
                temperature=configs.DEFAULT_LLM_TEMPERATURE,
                api_key=configs.OPENAI_API_KEY,
                max_tokens=configs.MAX_TOKENS,
                top_p=0.95 if configs.ENVIRONMENT == Environment.PRODUCTION else 0.8,
                presence_penalty=0.1 if configs.ENVIRONMENT == Environment.PRODUCTION else 0.0,
                frequency_penalty=0.1 if configs.ENVIRONMENT == Environment.PRODUCTION else 0.0,
            ),
        },
        {
            "name": "gpt-4o-mini",
            "llm": ChatOpenAI(
                ...
                api_key=configs.OPENAI_API_KEY,
                max_tokens=configs.MAX_TOKENS,
                top_p=0.9 if configs.ENVIRONMENT == Environment.PRODUCTION else 0.8,
            ),
        },
    ]
    ...
    def get(cls, model_name: str, **kwargs) -> BaseChatModel:
        ...
        if kwargs:
            ...
            return ChatOpenAI(model=model_name, api_key=configs.OPENAI_API_KEY, **kwargs)
...
class LLMService:
    ...
    def __init__(self):
        ...
        try:
            self._current_model_index = all_names.index(configs.DEFAULT_LLM_MODEL)
            self._llm = LLMRegistry.get(configs.DEFAULT_LLM_MODEL)
            logger.info(
                "llm_service_initialized",
                default_model=configs.DEFAULT_LLM_MODEL,
            ...
            logger.warning(
                ...
                requested=configs.DEFAULT_LLM_MODEL,
    ...
    @retry(
        stop=stop_after_attempt(configs.MAX_LLM_CALL_RETRIES),
