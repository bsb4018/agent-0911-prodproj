...
from src.agent.tools.tools_manager import tools
from src.utils.logging import logger
from src.utils.telemetry import llm_inference_duration_seconds
from src.agent.prompts.prompt_manager import load_system_prompt
from src.schemas.schema_chat import Message
from src.schemas.schema_graph import GraphState
from src.services.service_llm_provider import llm_service
...
class LangGraphAgent:
            ...
            self.memory = await AsyncMemory.from_config(
                config_dict={
                    "vector_store": {
                        "provider": "pgvector",
                        "config": {
                            "collection_name": configs.LONG_TERM_MEMORY_COLLECTION_NAME,
                    ...
                    "llm": {
                        "provider": "openai",
                        "config": {"model": configs.LONG_TERM_MEMORY_MODEL},
                    },
                    "embedder": {"provider": "openai", "config": {"model": configs.LONG_TERM_MEMORY_EMBEDDER_MODEL}},
    ...
    async def _get_connection_pool(self) -> AsyncConnectionPool:
        """Get a PostgreSQL connection pool using environment-specific configs.
        ...
        if self._connection_pool is None:
            try:
                ...
                max_size = configs.POSTGRES_POOL_SIZE

                connection_url = (
                    "postgresql://"
                    f"{quote_plus(configs.POSTGRES_USER)}:{quote_plus(configs.POSTGRES_PASSWORD)}"
                    f"@{configs.POSTGRES_HOST}:{configs.POSTGRES_PORT}/{configs.POSTGRES_DB}"
                )
                ...
                logger.info("connection_pool_created", max_size=max_size, environment=configs.ENVIRONMENT.value)
            except Exception as e:
                logger.error("connection_pool_creation_failed", error=str(e), environment=configs.ENVIRONMENT.value)
                # In production, we might want to degrade gracefully
                if configs.ENVIRONMENT == Environment.PRODUCTION:
                    logger.warning("continuing_without_connection_pool", environment=configs.ENVIRONMENT.value)
                    return None
                raise e
    ...
    async def _chat(self, state: GraphState, config: RunnableConfig) -> Command:
        ...
        model_name = (
            ...
            else configs.DEFAULT_LLM_MODEL
        )
        ...
        if self._graph is None:
            try:
                ...
                self._graph = graph_builder.compile(
                    checkpointer=checkpointer, name=f"{configs.PROJECT_NAME} Agent ({configs.ENVIRONMENT.value})"
                )

                logger.info(
                    "graph_created",
                    graph_name=f"{configs.PROJECT_NAME} Agent",
                    environment=configs.ENVIRONMENT.value,
                    has_checkpointer=checkpointer is not None,
                )
            except Exception as e:
                logger.error("graph_creation_failed", error=str(e), environment=configs.ENVIRONMENT.value)
    ...
    async def get_stream_response(
        ...
        config = {
            ...
            "callbacks": [
                CallbackHandler(
                    environment=configs.ENVIRONMENT.value, debug=False, user_id=user_id, session_id=session_id
                )
            ],
            "metadata": {
                "user_id": user_id,
    ...
    async def clear_chat_history(self, session_id: str) -> None:
        ...
        try:
            ...
            async with conn_pool.connection() as conn:
                for table in configs.CHECKPOINT_TABLES:
                    try:
