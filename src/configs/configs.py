...
class Configs:
    """Application Configs without using pydantic."""

    def __init__(self):
        """Initialize application Configs from environment variables.
        ...
        self.apply_environment_Configs()

    def apply_environment_Configs(self):
        """Apply environment-specific Configs based on the current environment."""
        env_Configs = {
        ...
        current_env_Configs = env_Configs.get(self.ENVIRONMENT, {})

        # Apply Configs if not explicitly set in environment variables
        for key, value in current_env_Configs.items():
